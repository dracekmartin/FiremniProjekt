...
\begin{itemize}
  ...
  \item \textbf{Licensing data migration} from an XML file into the company's central database, with the related internal tooling created or rewritten.
...
\begin{tikzpicture}[
  ...
  \node[c4database] (central) at (6, -4.5) {%
    \textbf{M-line central DB}\\{\footnotesize [Database]}\\{\scriptsize Licenses and updates,}\\{\scriptsize one for all customers}
  };
  ...
  \draw[c4arrow] (licenser.south) -- node[midway, right] {Maintains licensing data\\\textit{[internal tooling]}} (central.north);
  \draw[c4arrow] (edison2.south) .. controls (-6, -7.5) and (0, -7.5) ..
    node[pos=0.75, below] {Fetches license and updates\\\textit{[HTTP]}} (central.west);
  \draw[c4arrow] (legacy.south east) -- node[pos=0.4, above right=-2pt] {Fetches updates\\\textit{[HTTP]}} (central.north west);
\end{tikzpicture}
\caption{C4 System context diagram. EDISON 2, the focus of this project, runs next to the legacy client against the same customer Caché backend. Licenses and update data live in M-line's central database: a single instance serving all customers, physically separate from the customer backends but built on the same database technology. The licenser maintains licensing data there. Today the license is an XML file distributed with the client, and moving it to the central database is part of this project.}
...
\noindent \textbf{Shell}
\begin{itemize}
  \item \reqdef{S1} The shell authenticates users against the Caché backend with a username and password. The backend decides the authentication mode per user.
  \item \reqdef{S2} The user selects a company at login and can switch companies at runtime, which closes open agenda windows first.
  ...
  \item \reqdef{S6} Every still-relevant legacy WinForms agenda can be launched through a compatibility bridge that passes the established session and company context.
  ...
  \item \reqdef{S9} The shell verifies client/server version compatibility before and during a session and refuses to operate on a mismatch.
...
\begin{itemize}
  ...
  \item \reqdef{M3} Mini-apps are enabled per user, in the user's settings (see the description above).
...
\begin{itemize}
  \item \reqdef{L1} Licenses move from the XML file into M-line's central database: a single instance serving all customers, physically separate from the customer backends but built on the same database technology. Customer installations query it for their license, so changing a license no longer involves sending any file to the customer.
  \item \reqdef{L2} The shell consumes the license: it hides unlicensed modules, blocks logins over the concurrent-user limit, and shows non-blocking warnings for approaching limits and expiry.
  \item \reqdef{L3} An editor for the licenser allows creating, modifying, and issuing customer licenses, following the conventions of the company's newer internal tools.
  \item \reqdef{L4} The definition of module relationships (which elements belong to which licensable feature, where a module sits in the tree) is written by developers and lives in the customer backend. Developers get a supported way to edit it, replacing today's manual XML maintenance.
  \item \reqdef{L5} Existing licensing data is migrated from XML with verified equivalence: a customer installation sees the same modules before and after the migration.
...
\begin{itemize}
  \item \reqdef{U1} At startup the client compares its version with the server installation and, on a mismatch, performs a mandatory self-update: download, apply with a rollback on failure, restart.
...
\begin{itemize}
  ...
  \item \reqdef{N3} Testability: all public APIs of the toolkit and the shell are covered by unit tests on both stacks, and the tests run automatically as part of the development workflow.
  ...
  \item \reqdef{N7} Localization: the UI is Czech, with texts centralized so that other languages remain possible.
  ...
  \item \reqdef{N10} AI-assistant friendliness: in-repo guidance files, explicit and strongly typed dependency injection, no hidden global state, and a verification pipeline before accepting generated changes.
  \item \reqdef{N11} Security: licensing data is validated server-side and tamper-resistant, stored credentials are encrypted, and update packages are applied atomically with a rollback.
\end{itemize}
...
\begin{figure}[H]
\centering
\mockupbox{Mini-apps in use: mini-apps docked in the hosting area showing agenda summaries, and the user's settings dialog with the list of available mini-apps and per-user toggles.}
\caption{Mini-apps in the main window, with the per-user opt-in.}
...
\medskip
\noindent The licensing editor does not exist yet even in a legacy form worth showing, so instead of a screenshot, a short storyboard: the licenser opens the tool and sees the list of customers and their licenses with validity. Selecting a license shows the purchased feature set as a tree of licensable modules with checkboxes, the numeric limits, and the validity period. Saving stores the license in the M-line central database, from where the customer installation reads it. No file is sent to the customer manually. A related view for developers allows editing the module relationships (which elements belong to which licensable feature), replacing today's manual XML editing.
...
\begin{tikzpicture}[
  ...
  \node[c4database] (central) at (-8, -8) {%
    \textbf{M-line central DB}\\{\footnotesize [Database]}\\{\scriptsize Licenses and}\\{\scriptsize update packages}
  };
  ...
  \draw[c4arrow] (shell.west) .. controls (-11.5, 4) and (-11.5, -3) ..
    node[pos=0.45, left] {Fetches license\\and updates\\\textit{[HTTP]}} (central.west);
  \draw[c4arrow] (legacy.south) -- node[pos=0.5, right] {Downloads\\updates\\\textit{[HTTP]}} (central.north);
\end{tikzpicture}
\caption{C4 Container diagram for the EDISON product line. EDISON~2 Core (modernized shell, shared toolkit, legacy bridge) is the focus of this project; TemplateDesigner is a peer WPF agenda within EDISON~2 that also consumes the toolkit. The legacy bridge spawns existing WinForms agendas, which are also launched by the legacy shell. All systems share the customer's Cach\'e backend, which after the migration also holds the module definitions. Licenses and update packages come from the M-line central database, a single instance serving all customers.}
...
\medskip
\noindent \textbf{Automated testing.} All public APIs of the toolkit and the shell ship with unit tests on both stacks, and the tests run automatically as part of the development workflow (\req{N3}).
...
\begin{itemize}
  ...
  \item \textbf{Licensing migration underestimated.} The XML format has grown over the years and part of its semantics exists only in the code that reads it. Mitigation: the migration is verified by the equivalence check from \req{L5}, and the data model comes before the editor. Fallback: legacy clients keep reading the XML file either way, so the migration can be re-run until it is right.
...
\begin{itemize}
  ...
  \item \textbf{M2 Toolkit foundation} -- the core components, theming, and backend access, exercised by a first minimal version of the shell.
  \item \textbf{M3 Shell prototype} -- login, module tree, and launching legacy agendas, all in daily use on developer machines.
  ...
  \item \textbf{M5 Licensing migration} -- licenses served from the central database, migration verified, tooling usable by the licenser and developers.
  \item \textbf{M6 Update mechanism} -- the new client updates itself from the company's distribution channel.
  \item \textbf{M7 Pilot deployment} -- all developer workstations on the new shell by default and a customer pilot site using only the new shell.
\end{itemize}
...
\begin{tabular}{ | m{11em} | m{23em} | m{2.5cm} | m{1.3cm} | }
 ...
 \hline
 M3 Shell prototype & Login, module tree, legacy bridge, daily internal use & 8 -- 10/2026 & 24 \\
 \hline
 ...
 \hline
 M6 Update mechanism & Design, client and server implementation & 11 -- 12/2026 & 14 \\
 \hline
